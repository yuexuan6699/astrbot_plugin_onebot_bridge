...
async def recall_send(delay: int, event: AstrMessageEvent, message_chain: MessageChain) -> None:
    ...
    try:
        ...
        if platform_name != "aiocqhttp":
            logger.debug("[撤回] 非aiocqhttp平台，使用普通发送")
            await event.send(message_chain)
            return
        
        from astrbot.core.platform.sources.aiocqhttp.aiocqhttp_message_event import AiocqhttpMessageEvent
        if not isinstance(event, AiocqhttpMessageEvent):
            logger.debug("[撤回] 事件类型不是AiocqhttpMessageEvent")
            await event.send(message_chain)
            return
        ...
        if message_id:
            ...
            logger.warning("[撤回] 未获取到message_id，无法撤回")
